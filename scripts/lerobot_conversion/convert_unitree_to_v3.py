...
print("THIS CODE IS NOT MEANT TO BE RUN DIRECTLY. IT IS A TEMPLATE FOR CONVERTING UNITREE XR_TELEOP JSON DATA TO LEROBOT V3 FORMAT. ")
print("RUN THE CODE FROM THE UNITREE_LEROBOT REPO")
...
class JsonDataset:
    def __init__(self, data_dirs: Path, robot_type: str) -> None:
        """
        Initialize the dataset for loading and processing JSON files containing
    ...
    def _init_paths(self) -> None:
        """Initialize episode paths by searching for data.json files."""
        self.episode_paths = []
        self.task_paths = []

        # Support both layouts: task/episode/ and flat episode_* dirs
    ...
    def _parse_images(self, episode_path: str, episode_data) -> dict[str, list[np.ndarray]]:
        ...
        for camera in cameras:
            ...
            for sample_data in episode_data["data"]:
                ...
                images[image_key].append(image_rgb) #Each camera key contains a list of images
    ...
    def get_item(
        ...
        if index is None:
            # For our conversion script we always pass an explicit index,
            # but keep this behavior for completeness.
...
def create_empty_dataset(
    ...
    *,
    ...
    # Images are stored as HWC uint8; dtype is "video" or "image"
    for cam in cameras:
    ...
    # Match use_videos to mode: only write videos when mode == "video"
...
def populate_dataset(
    ...
    for i in tqdm.tqdm(range(len(json_dataset))):
        ...
        expected_cameras = ROBOT_CONFIGS[robot_type].cameras
        missing_cameras = [cam for cam in expected_cameras if cam not in cameras]
        if missing_cameras:
            raise ValueError(
        ...
        task_str = str(task) if task else "default"
        ...
        for t in range(num_frames):
            ...
            for camera, img_array in cameras.items():
                frame[f"observation.images.{camera}"] = img_array[t]
...
def json_to_lerobot(
    ...
    *,
    ...
    # Ensure parquet writers close and write footers (magic bytes) before optional upload
    dataset.finalize()
