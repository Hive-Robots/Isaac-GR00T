...
def eval(cfg: EvalConfig):
    ...
    try:
        ...
        target_arm_q = np.asarray(current_arm_q, dtype=np.float32).copy()
        target_side_states = _read_ee_side_states(bool(cfg.ee), ee_shared_mem, ee_dof, ee_sides)
        target_waist_yaw = _read_current_waist_yaw(arm_ctrl)
        if cfg.dataset_path:
            init_pose = _read_initial_pose_from_dataset(cfg.dataset_path)
            target_arm_q = init_pose["arm_q"].astype(np.float32)
            target_side_states["left"] = init_pose["left_hand"].astype(np.float32)
            target_side_states["right"] = init_pose["right_hand"].astype(np.float32)
            if init_pose["waist_yaw"] is not None:
                target_waist_yaw = float(init_pose["waist_yaw"])
        ...
        if hasattr(arm_ctrl, "ctrl_waist_yaw_abs"):
            arm_ctrl.ctrl_waist_yaw_abs(float(target_waist_yaw))
        if bool(cfg.ee) and ee_dof > 0:
            if isinstance(ee_shared_mem["left"], SynchronizedArray):
                ee_shared_mem["left"][:] = to_list(
                    target_side_states.get("left", np.zeros(ee_dof, dtype=np.float32))
                )
                ee_shared_mem["right"][:] = to_list(
                    target_side_states.get("right", np.zeros(ee_dof, dtype=np.float32))
                )
            elif hasattr(ee_shared_mem["left"], "value") and hasattr(ee_shared_mem["right"], "value"):
