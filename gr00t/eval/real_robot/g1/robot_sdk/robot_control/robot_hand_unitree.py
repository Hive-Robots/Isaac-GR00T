...
class Dex3_1_Controller:
    def __init__(
        ...
        self.LeftHandCmb_publisher = (
            ChannelPublisher(kTopicDex3LeftCommand, HandCmd_) if "left" in self.enabled_sides else None
        )
        if self.LeftHandCmb_publisher:
            self.LeftHandCmb_publisher.Init()
        self.RightHandCmb_publisher = (
            ChannelPublisher(kTopicDex3RightCommand, HandCmd_) if "right" in self.enabled_sides else None
        )
        if self.RightHandCmb_publisher:
            self.RightHandCmb_publisher.Init()

        self.LeftHandState_subscriber = (
            ChannelSubscriber(kTopicDex3LeftState, HandState_) if "left" in self.enabled_sides else None
        )
        if self.LeftHandState_subscriber:
            self.LeftHandState_subscriber.Init()
        self.RightHandState_subscriber = (
            ChannelSubscriber(kTopicDex3RightState, HandState_) if "right" in self.enabled_sides else None
        )
        if self.RightHandState_subscriber:
        ...
        while True:
            ...
            if "left" in self.enabled_sides:
                ready.append(any(self.left_hand_state_array))
            if "right" in self.enabled_sides:
                ready.append(any(self.right_hand_state_array))
    ...
    def _subscribe_hand_state(self):
        while True:
            left_hand_msg = self.LeftHandState_subscriber.Read() if self.LeftHandState_subscriber else None
            right_hand_msg = self.RightHandState_subscriber.Read() if self.RightHandState_subscriber else None
            if "left" in self.enabled_sides and left_hand_msg is not None:
                for idx, id in enumerate(Dex3_1_Left_JointIndex):
                    self.left_hand_state_array[idx] = left_hand_msg.motor_state[id].q
            if "right" in self.enabled_sides and right_hand_msg is not None:
    ...
    def ctrl_dual_hand(self, left_q_target, right_q_target):
        """Set current hand motor targets for enabled sides."""
        if "left" in self.enabled_sides and self.left_msg and self.LeftHandCmb_publisher:
            for idx, id in enumerate(Dex3_1_Left_JointIndex):
                self.left_msg.motor_cmd[id].q = left_q_target[idx]
            self.LeftHandCmb_publisher.Write(self.left_msg)
        if "right" in self.enabled_sides and self.right_msg and self.RightHandCmb_publisher:
            for idx, id in enumerate(Dex3_1_Right_JointIndex):
    ...
    def control_process(
        ...
        self.left_msg = unitree_hg_msg_dds__HandCmd_() if "left" in self.enabled_sides else None
        if self.left_msg:
        ...
        self.right_msg = unitree_hg_msg_dds__HandCmd_() if "right" in self.enabled_sides else None
        if self.right_msg:
        ...
        try:
            while self.running:
                ...
                # get dual hand state
                left_hand_mat = np.zeros(Dex3_Num_Motors)
                right_hand_mat = np.zeros(Dex3_Num_Motors)
                if "left" in self.enabled_sides:
                ...
                state_parts = []
                if "left" in self.enabled_sides:
                    state_parts.append(np.array(left_hand_state_array[:]))
                if "right" in self.enabled_sides:
                    state_parts.append(np.array(right_hand_state_array[:]))
                state_data = np.concatenate(state_parts) if state_parts else np.array([])

                # get dual hand action
                action_parts = []
                if "left" in self.enabled_sides:
                    action_parts.append(left_hand_mat)
                if "right" in self.enabled_sides:
                    action_parts.append(right_hand_mat)
                action_data = np.concatenate(action_parts) if action_parts else np.array([])
